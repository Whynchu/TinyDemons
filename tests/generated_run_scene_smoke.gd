...
func _initialize() -> void:
	...
	_expect(packed != null, "main scene loads for the generated Run 7 flow", failures)
	if packed == null:
		_finish(failures)
		return
	...
	if graph != null and map != null and rooms != null and minimap != null:
		map.call("begin_run", graph, 182736, 6, &"water")
		...
		_expect(not bool(map.call("is_authored_layout")) and bool(map.call("has_complete_layout")), "Run 7 uses the generated complete-layout path after authored Run 6", failures)
		_expect(bool(minimap.get("visible")), "generated Run 7 displays its generated minimap", failures)
		...
		# Run 7's alternate Fire Room must be a real palette source, not just a
		...
		for room_id in graph.get_room_ids():
			...
			if candidate != null and candidate.room_type == GRAPH_SCRIPT.ROOM_FIRE and candidate.fire_flame == &"fire":
				...
				break
		_expect(alternate_fire_room != null, "generated Run 7 exposes a declared alternate Fire Room", failures)
