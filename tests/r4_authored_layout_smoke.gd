...
func _initialize() -> void:
	...
	for rotation in range(4):
		var rotated_layout = RUN3_LAYOUT_SCRIPT.build(&"water", rotation)
		var rotated_validation: Array[String] = rotated_layout.validate()
		_expect(rotated_validation.is_empty(), "R3 rotation %d keeps every authored room reachable" % rotation, failures)
	...
	_expect(layout.connections.size() == 44, "R3 compiles every one of its 44 gate pixels", failures)
	...
	_expect(normal_count == 28, "R3 preserves 28 ordinary grey entrances", failures)
