...
func _initialize() -> void:
	...
	_expect(actor_pool[0].texture != null and actor_pool[0].self_modulate.a > 0.0, "replacement skeleton starts with visible artwork", failures)
	...
	_expect(not occlusion_renderer.original_actor_scales.has(actor_slot) and occlusion_renderer.original_actor_scales.get(actor_pool[0]) == Vector2.ONE, "family replacement starts from the new actor's authored scale", failures)
	_expect(actor_pool[0].scale == Vector2.ONE, "family replacement drops the prior slime's transient stretch", failures)
