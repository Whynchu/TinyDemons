...
func _initialize() -> void:
	...
	_expect(layout.connections.size() == 43, "R3 compiles every one of its 43 gate pixels", failures)
	...
	_expect(normal_count == 27, "R3 preserves 27 ordinary grey entrances", failures)
