...
func _initialize() -> void:
	...
	_expect(chroma.restore_neutral_chroma(), "Gray can gain neutral Chroma", failures)
	_expect(chroma.current_chroma == 20, "Gray pickup fills the neutral Chroma bar", failures)
	_expect(chroma.current_aspect == Chroma.Aspect.NONE and chroma.ability_mode() == Chroma.AbilityMode.GRAY, "neutral pickup does not invent an elemental identity", failures)
	chroma.begin_new_run()
	...
	_expect(chroma.chroma_palette_name() == "yellow", "bound zero keeps the Electric visual identity", failures)
