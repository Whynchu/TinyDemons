...
static func calculate_damage(
	...
	attacker_damage_rate_bonus: float,
	...
	var raw_damage := config.damage_base + attacker_str
	raw_damage *= 1.0 + maxf(attacker_damage_rate_bonus, 0.0)
...
static func max_health_for_stats(stats: StatsComponent, equipment_health_rate_bonus: float = 0.0, tuning: CombatTuning = null) -> float:
	if stats == null:
	...
	snapshot.gear_health_rate = equipment_health_rate_bonus
...
static func max_health_for_snapshot(snapshot: CombatStatSnapshot, tuning: CombatTuning = null) -> float:
	...
	return calculated_health * (1.0 + maxf(snapshot.gear_health_rate, 0.0))
...
static func calculate_snapshot_damage(attacker: CombatStatSnapshot, defender: CombatStatSnapshot, can_critical: bool, rng: RandomNumberGenerator, tuning: CombatTuning = null) -> DamageResult:
	...
	if attacker == null:
		return result
	var raw_damage := config.damage_base + float(attacker.strength)
	raw_damage *= 1.0 + maxf(attacker.gear_damage_rate, 0.0)
