...
func update_player_shadow(root: Object, depth_scale: float) -> void:
	...
	shadow.global_position = root.call("_actor_foot", player) + root.get("player_shadow_offset"); shadow.global_scale = root.get("player_shadow_scale"); shadow.self_modulate = Color(1, 1, 1, 0.25); shadow.flip_h = player.flip_h; shadow.z_index = z_index_for(root.call("_actor_foot", player).y, depth_scale)
