...
static func build(selected_starter_flame: StringName = &"fire", rotation_quarter_turns: int = 0):
	...
	var alternates: Array[StringName] = ASPECT_CATALOG_SCRIPT.alternate_flames_for_run(5, starter_flame)
	var run_flame: StringName = alternates[0] if not alternates.is_empty() else starter_flame
	return COMPILER_SCRIPT.build_r6(starter_flame, run_flame, rotation_quarter_turns)
