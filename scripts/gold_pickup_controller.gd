extends Node
class_name GoldPickupController

## Owns denomination gold pickup instances and their transient launch state.
...
func snapshot() -> Array:
	...
	for index in sprites.size():
		...
		if air_times[index] > 0.0 and sprite.has_meta("gold_landing_position"):
			position = sprite.get_meta("gold_landing_position") as Vector2
		result.append({"position": position, "value": values[index]})
	return result
