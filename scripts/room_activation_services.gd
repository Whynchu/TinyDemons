...
var clear_gold_pickups: Callable = Callable()
...
var restore_gold_pickups: Callable = Callable()
...
static func from_runtime(runtime: GameplayState, controller: RoomController, state: Dictionary, room_type: StringName) -> RoomActivationServices:
	...
	services.clear_gold_pickups = Callable(runtime.pickup_runtime_controller, "clear_gold_pickups").bind(runtime)
	...
	services.restore_gold_pickups = Callable(runtime.pickup_runtime_controller, "restore_gold_pickups").bind(runtime, state.get("gold_pickups", []))
