...
const R1_BOSS_HEALTH_FACTOR := 1.0
const BOSS_ENCOUNTER_HEALTH_FACTOR := 1.0
...
func apply_enemy_room_level(root: Object, slime: Sprite2D, level_override: int = 0) -> void:
	...
	if stats == null:
		return
	stats.set_enemy_progression_tuning(root.get("progression_tuning") as ProgressionTuning)
	var run := root.get("run_state") as RunState
