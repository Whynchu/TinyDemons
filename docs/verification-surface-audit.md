...
| `web_export_smoke` | `role:gate` + `state:verified` | Single-threaded Compatibility export now passes in a fresh standalone output directory; Pages still receives an explicit `dist` output path | Keep the export in the default gate; browser/device and hosted-artifact checks remain separate |
| `touch_controls_smoke` | `role:gate` + `state:verified` | The focused contract now supplies the optional pause callbacks, uses the current stat-row geometry, and distinguishes inert menu blanks from dialogue accept | Keep one bounded input contract; do not add a second test file for the same path |
