...
- New enemy and room content is not yet fully definition/factory driven.
...
Build an `EnemyDefinition` + `EnemyFactory` vertical proof with one additional enemy sharing the existing component architecture and requiring zero `GameplayState` edits. Then apply the same pattern to room and dungeon definitions.
